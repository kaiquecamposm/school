...
def registers_for_value_filter(registers, min_value):
   filtered_registers = [register for register in registers if register["value"] >= min_value]
...
def remove_prev_registers(registers, limit):
  updated_registers = [register for register in registers if limit <= register["timestamp"]] 
...
def main():
  ...
  for register in registers[:5]:
    print(f"ID: {register["id"]} | Timestamp: {register["timestamp"]} | Valor: {register["value"]} | Dispositivo: {register["id_dispositivo"]}")
