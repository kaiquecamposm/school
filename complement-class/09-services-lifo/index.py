...
def gen_requests(num_requests):
  ...
  problems = ["Problema de conexão", "Erro no sistemas", "Dúvida sobre faturamento", "Solicitação de suporte"]
...
def process_request(queue_requests):
  if (queue_requests):
    request = queue_requests.pop()
    print(f"Processando requisição ID: {request["id"]} | Cliente: {request["cliente"]} | Problema: {request["problema"]} | Horario: {request["horario"]}")
    time.sleep(1)
  else:
